...
import type { TranscriptJobPayload, EmbeddingJobPayload, SummaryJobPayload } from '@app/queue';
...
export class TranscriptProcessor extends WorkerHost {
  ...
  async process(job: Job<TranscriptJobPayload>): Promise<void> {
    ...
    try {
      // Extend lock manually before heavy Whisper call
      ...
      const summaryPayload:   SummaryJobPayload   = { jobId, transcriptId, rawText };
      ...
      this.logger.log(`[${jobId}] Transcript done → embedding + summary queued`);
